"""Kodowanie i dekodowanie payloadów ramek TCMP - jedno źródło prawdy.

Każda funkcja ``encode_*`` zwraca surowy payload (bez nagłówka 49B) gotowy do
przekazania do ``frame.build_frame``, a każda ``decode_*`` przyjmuje payload
zwrócony przez ``frame.parse_frame``["payload"].

Moduł jest współdzielony przez klienta, serwer i narzędzia (np. seeding bazy),
aby format payloadu istniał wyłącznie w jednym miejscu i nie mógł się rozjechać
między komponentami.
"""
import struct

from . import constants as c
from .frame import pack_string, unpack_string
...
# AUTH_OK (0x03)  S -> C
# --------------------------------------------------------------------------- #
def encode_auth_ok(session_token: str, session_key: bytes, queued_messages: int) -> bytes:
    return (
        pack_string(session_token)
        + struct.pack("!H", len(session_key)) + session_key
        + struct.pack("!I", queued_messages)
    )
